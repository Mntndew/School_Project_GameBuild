        ...
        public void Attack(Game1 game, GameTime gameTime, H_Map.TileMap tiles)
        {
            ...
            if (Game1.character.position.Intersects(position) && Game1.map.mapName.Remove(Game1.map.mapName.Length - 1) == mapName && !Game1.character.dead)
            {
                if (attackTimer <= 0)
                {
                    ...
                    Game1.particleSystemManager.particleSystem[0].Add(Game1.character.position.X + (Game1.character.position.Width / 2), Game1.character.position.Y + 32, 4, 4, damage, -4, 4, 0, 2, new Color(255, 0, 0), 0.2f, 1, 1, false, false, true);
